...
void send_all_acceptable_packages(town* source, int source_office_index, town* target, int target_office_index) {
  ...
  package* accepted = malloc(sizeof(package) * accepted_count);
  package* rejected = malloc(sizeof(package) * src->packages_count - accepted_count);
  int a_idx = 0; int  r_idx = 0;
  ...
  for (int i =0; i < src->packages_count; i++) {
     if (src->packages[i].weight >= dst -> min_weight &&  src->packages[i].weight <= dst -> max_weight) {
         ...
         src->packages_count -= accepted_count;
         dst->packages = realloc(dst->packages, sizeof(package)*(dst->packages_count + accepted_count));
          for (int i = 0; i < accepted_count; i++) {
            dst->packages[dst->packages_count + i] = accepted[i];
          }
        dst->packages_count += accepted_count;  
